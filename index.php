<?php
//把https://github.com/modood/Administrative-divisions-of-China四级联动（https://raw.githubusercontent.com/modood/Administrative-divisions-of-China/master/dist/pcas.json）转为数据库
//error_reporting(0);
define('PRE','je_');
...
$china=json_decode(file_get_contents('./4.json'),true);
...
function db($table='',$prefix=''){
    static $db=null;
	if(!$db){
		$db = new Connection(HOST, USER, PSSS, DB);
	}
	if($table!==''){
		if($prefix!==NULL){
			$table=$prefix==='' ? 'je_'.$table : $prefix.$table;
		}
		$db->setTable($table);
...
//use Exception;use PDO;use PDOException;
/**
	   ...
	   try {
			...
			if ($e->errorInfo[1] == 2006 || $e->errorInfo[1] == 2013) {
				...
				try {
					...
					$this->success = $this->sQuery->execute();
					if(defined('APP_DEBUG') && APP_DEBUG  ) file_put_contents('conn.txt','[重连]'.$query."\r\n".print_r($this->parameters,1)."\r\n",FILE_APPEND);
